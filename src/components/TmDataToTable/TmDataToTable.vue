...
const multipleTableRef = ref<TableInstance>(); // 表格实例
const selectedTableData = defineModel<Array<TimeIntervalObject>>(); // 选中的表格数据

const tableRowClassName = function ({
  row,
  rowIndex,
}: {
  row: TimeIntervalObject;
  rowIndex: number;
}) {
  // highLight
  let classStr = "";
...
// 单击行同样进行勾选
const handleSelectionChange = (val: Array<TimeIntervalObject>) => {
  selectedTableData.value = val;
};

const handleRowClick = (row: TimeIntervalObject) => {
